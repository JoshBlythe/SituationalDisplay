...
{
    ref(WsTcpSocket) sc = NULL;
    //ref(WsTcpSocket) sc;

    sc = allocate(WsTcpSocket);

    _(sc).id++;

    if(_(sc).initalised) return sc;

    _(sc).initalised = true;


    int res = 0;

#ifdef USE_WINSOCK
    res = WSAStartup(MAKEWORD(2,2), &_(sc).wsaData);
    ...
    res = getaddrinfo(host, portString, &server, &result);


    if(res != 0)
    {
        _WsPanic("Socket Invalid!");
        //return;
    }

    _(sc).fd = INVALID_SOCKET;

    for (struct addrinfo *ptr = result; ptr != NULL; ptr = ptr->ai_next)
    {
        _(sc).fd = socket(ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol);
        if(_(sc).fd == INVALID_SOCKET)
        {
            _WsPanic("Socket Invalid!");
            //return;
        }

        res = connect(_(sc).fd, ptr->ai_addr, (int)ptr->ai_addrlen);
    ...
    printf("TcpSocket Connected [%s:%i]\n", host, port);
    _(sc).connection = true;
    _(sc).state = 2;
...
#ifdef USE_WINSOCK
  bytes = send(_(ctx).fd, &vector_at(data, 0), vector_size(data), 0);
#endif
#ifdef USE_POSIX
